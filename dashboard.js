...
(function () {
  ...
  /* ---------------------------------------------------------
     RECENT ACTIVITY — merged feed across saved routes, quote
     submissions/status changes, and order status changes.
     Each loader below calls pushActivitySource() with its own
     latest items; we re-render the merged, time-sorted feed
     every time any source updates.
  --------------------------------------------------------- */
  function relTime(ts) {
  ...
  function renderActivity() {
    ...
    activitySources.quotes.forEach(function (q) {
      var label = EGC.quoteStatusLabel(q.status);
      merged.push({ ts: q.updatedAt || q.createdAt, ico: ICO_QUOTE, txt: 'Quote ' + q.quoteId + ' — ' + label });
    });
    activitySources.orders.forEach(function (o) {
      var label = EGC.orderStatusLabel(o.status);
      merged.push({ ts: o.updatedAt || o.createdAt, ico: ICO_ORDER, txt: 'Order ' + o.orderId + ' — ' + label });
    });
  ...
  /* ===========================================================
     PHASE 3 — MODULE 1: NEW SHIPMENT / QUOTE REQUEST
     Customer submits pickup/delivery/material/weight/etc.
     Saved to quotes/{quoteId} with status = pending_review.
  =========================================================== */
  ...
  /* ===========================================================
     ...
     Real-time listener on quotes where customerUid == me.
  =========================================================== */
  ...
  /* ===========================================================
     ...
     Real-time listener on orders where customerUid == me.
  =========================================================== */
  ...
  /* Empty-state CTAs inside dynamically-rendered lists need their
     [data-tab-link] click handler re-attached after innerHTML swaps,
     since the original wiring only ran once at page load. */
