/* ============================================================
   ...
   - Recent Activity feed from Firestore activityLog
...
(function () {
  ...
  function listenQuotes(uid) {
    return fbDB.collection('quotes')
      ...
      .onSnapshot(function (snap) {
        ...
        console.error('[DASH] quotes listener:', err.code, err.message);
      });
  }

  /* Recent Quotes widget on home panel */
  function renderRecentQuotes(quotes) {
    var list = $('#recentQuotesList');
    if (!list) return;
    ...
    if (!recent.length) {
      list.innerHTML = (
        '<div class="empty-widget">' +
          ...
          '<p>No quotes yet. <a href="#" onclick="document.getElementById(\'newQuoteTab\').click();return false;">Request your first quote</a>.</p>' +
        '</div>'
      );
      return;
    }
    list.innerHTML = recent.map(function (q) {
      ...
      var revisedBanner = (q.status === 'revised_by_owner')
        ? '<div class="revised-action-banner">Revised quote — <a href="#" onclick="openTab(\'quotes\');return false;">Review &amp; respond</a></div>'
        : '';
      return (
        '<div class="qrow">' +
          ...
          '<div class="qrow-route">' +
            ...
            '<span>' + EGC.esc(q.pickup) + ' \u2192 ' + EGC.esc(q.delivery) + '</span>' +
          '</div>' +
          '<div class="qrow-right">' + badge + '<span class="qrow-date">' + EGC.fmtDate(q.createdAt) + '</span></div>' +
          revisedBanner +
        '</div>'
      );
    }).join('');
  ...
  function listenOrders(uid) {
    return fbDB.collection('orders')
      ...
      .onSnapshot(function (snap) {
        ...
        console.error('[DASH] orders listener:', err.code, err.message);
      });
  }
  ...
  function renderOrderHistory(orders) {
    ...
    list.innerHTML = orders.map(function (o) {
      ...
      var pct   = idx < 0 ? 0 : Math.round(((idx + 1) / seq.length) * 100);
      var progressBar = (
        '<div class="order-progress">' +
          seq.map(function (s, i) {
            var cls = i <= idx ? 'op-step done' : 'op-step';
            return '<div class="' + cls + '" title="' + EGC.orderStatusLabel(s) + '"><span class="op-label">' + EGC.orderStatusLabel(s) + '</span></div>';
          }).join('<div class="op-connector"></div>') +
        '</div>'
      );
  ...
  function listenNotifications(uid) {
    return fbDB.collection('notifications')
      ...
      .onSnapshot(function (snap) {
        ...
        console.error('[DASH] notifications listener:', err.code, err.message);
      });
  }
  ...
  function listenActivity(uid) {
    return fbDB.collection('activityLog').doc(uid).collection('entries')
      .orderBy('createdAt', 'desc')
      .limit(20)
      .onSnapshot(function (snap) {
        var entries = [];
